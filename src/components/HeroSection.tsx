...
export default function HeroSection() {
  ...
  const words: string[] = useMemo(
    () => safeJsonParse(heroData?.typed_words, DEFAULT_WORDS),
    [heroData]
  );
  const stats: Stat[] = useMemo(() => {
    const parsed = safeJsonParse(heroData?.stats, DEFAULT_STATS);
    return Array.isArray(parsed) ? parsed : DEFAULT_STATS;
  }, [heroData]);
  const cta: CTA = useMemo(
    () => safeJsonParse(heroData?.cta, DEFAULT_CTA),
    [heroData]
  );
  const features: Feature[] = useMemo(() => {
    const parsed = safeJsonParse(heroData?.features, []);
